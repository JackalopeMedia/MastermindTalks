...
/**
 *
 * Function used to generate post meta data aside block
 *
 * @param attachment - for the attachment size the script generate additional informations
 *
 * @return null
 *
 **/
function gk_post_aside($attachment = false) {
 	global $tpl;
...
?>
	<?php if($param_aside) : ?>
 	<aside class="meta">
 		<?php if($param_author) : ?>
	 	<a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author" title="<?php printf( __( 'Author: %s %s', GKTPLNAME ), get_the_author_meta('first_name', get_the_author_meta( 'ID' )), get_the_author_meta('last_name', get_the_author_meta( 'ID' )) ); ?>">
	 	...
	 	<ul>
	 		...
	 		<li class="category">
	 			<p><?php _e('Published in:', GKTPLNAME); ?></p>
	 			<?php echo get_the_category_list( __(', ', GKTPLNAME )); ?>
	 		</li>
	 		<?php endif; ?>
